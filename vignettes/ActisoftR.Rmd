...
vignette: >
  ...
  nocite: @ActisoftR, @rprog, @dplyr, @ggplot2, @lubridate, @plotly, @DT , @RColorBrewer,
---
...
* multiple activities: rest, sleep, active and excluded periods. 
...
* Often, actigraphy data comes from multiple wristwatches brands and versions. These files tend to differ in formats, variables, etc; which becomes more complex when dealing with large datasets.
* To best of our knowledge, there are not R functions available for effectively processing and visualizing this sort of dataset.


The package consists of several functions for importing, generating reports, summary statistics, and for data visualization.  
It handles output files from several actigraphs including ActionW (Ambulatory Monitoring, Inc.) and Actiware (Philips Respironics). 
...
The package consists of several functions for importing, generating reports, summary statistics, and for data visualization.  
It handles output files from several actigraphs including ActionW (Ambulatory Monitoring, Inc.) and from Actiware (Philips Respironics). 
...
load("C:\\1\\ActisoftR\\data\\act.RData")

act$datime_start <- ymd_hms(act$datime_start, tz = "UTC")
act$datime_end <- ymd_hms(act$datime_end, tz = "UTC" )
```
...
For example, the folder EXAMPLE_DATA contains Actiware and AMI-type files which will be simultaneously imported. 
...
Events such as rest episodes with no sleep, naps, jet lag, multiple sleep episodes, can be identified. 
Differences in sleep patterns between participants are also easily spotted. 


The package provides wide (```plot_Darwent```) and long type of plots (```plot_long```). 
The Darwent plot displays side-by-side multiple subjects, matching them by default in the left part of the plot. 
Long plots are suitable for a detailed with-in participants assessment. 
...
Metrics such as sleep efficiency and total sleep across intervals of times are relevant in sleep research.
*ActisoftR* generates multiple reports based on summary_type (sequential, daily, first, and time_to_time) and user-defined periods. 
...
Often sleeps/rest episodes are split based on the cut-off points. In this case, we use ```00:00``` in UTC.  
Sleep efficiency and other metrics are then computed for the defined period. The default computation is based on the weighted average.

The computed statistics will also depend on excluded periods e.g. when actigraph is removed from the wrist. A detailed set of rules is given in the package documentation. 
...
This is generally considered baseline sleep. Home sleep as seen in the above long plot was from ```2017-12-05``` to ```2017-12-15```.
...
Cumulative sleep debt refers to an accumulated deficit regarding the baseline or normal sleep. Baseline sleep is generally computed under common causes of variation e.g. at home under normal conditions.
A negative *csd* value means that on average the total sleep is smaller than the baseline value. 

            
The sleep debt across participants is computed using the ```csd``` function. The main arguments are the total sleep in the period of interest, the participant's baseline and optional a reset rule. This rule refers to the assumption that the sleep deficit resets to zero after a given consecutive number sleeps at night in the physiological time zone with similar or more sleep than baseline. This number is usually assumed to be equal to two.
...
*ActisoftR* is a user-friendly package that handles input data files from different actigraphs. 
...
We thank Philips Respironics for kindly allowing the use of Actiware file outputs. 
