...
report_period <- function(period, acti_data, remove_bad = TRUE, tz = "UTC", ...){
  ...
  startend <- as_tibble(int_startend(period))
  ...
  acti_data <- as_tibble(dplyr::filter(acti_data, subject_ID %in% particip, interval_type %in% c("REST", "SLEEP", "EXCLUDED", "FORCED SLEEP", "FORCED WAKE", "CUSTOM")))
  ...
  for (ii in 1 : length(particip)){
    ...
    for (jj in 1 : nrow(tab3)){
      ...
      if(any(mat0$datime_start == mat0$datime_end)){ # counter for sleeps with 0 duration
        zero_sleeps <- nrow(mat0[mat0$datime_start == mat0$datime_end,])
      } else{zero_sleeps <- 0}
      ...
      report$number_of_sleeps_exact <- as.numeric(as.character(mat2$number_exact[2])) - zero_sleeps
      report$number_of_rests <-  nrow(matex)
      report$number_of_sleeps <- mat2$interval_number[2] - zero_sleeps
