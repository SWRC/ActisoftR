...
#'
#' @export
#' @importFrom dplyr group_by slice all_of
#' @importFrom magrittr %>%
...
y$summary_end_datime <- maxi
  ...
  from <- y %>%
    group_by(subject_ID) %>%
    slice(which.min(summary_start_datime)) %>%
    select(all_of(c("subject_ID", "summary_start_datime")))

  to <- y %>%
    group_by(subject_ID) %>%
    slice(which.max(summary_end_datime)) %>%
    select(all_of(c("subject_ID", "summary_end_datime")))
