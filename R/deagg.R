...
deagg <-  function(period){ #deaggregation
  ...
  for (ii in 1 : length(particip)){
    ds0 <- ds <- matr <- NULL
    ds0 <- as_tibble(filter(period, subject_ID == particip[ii], summary_type %in% c("first", "time_to_time")))
    ds <- as_tibble(filter(period, subject_ID == particip[ii], summary_type %in% c("sequential", "daily"))) #daily and sequential
