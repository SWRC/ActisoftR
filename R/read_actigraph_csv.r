...
interval_type <- rem_tag <- NULL
  ...
  #ESF: In SLEEP, if end < start: replace end by start
  alldata2[alldata2$interval_type ==  'SLEEP' & alldata2$datime_start > alldata2$datime_end,]$datime_end <-
    alldata2[alldata2$interval_type ==  'SLEEP' & alldata2$datime_start > alldata2$datime_end,]$datime_start
